...
public:
	Plane(Point ref, Vector i, Vector j, float xMax, float yMax) : leftCorner(ref), xMax(xMax), yMax(yMax){
...
public:
	Stork() :overallState(STEP), leftLegState(1), rightLegState(2), forward(0), up(0), turnAngle(0), turnState(0), prevPosition(Point(0, 0, 0)), walkDir(Vector(1, 0, 0)), moveDown(true), turnLeft(true){
		...
		Point sPTop = storkbody->getMidlinePoint(0); Point sPBottom = storkbody->getTurnPointPosition();
	...
	void draw(bool displayShadow){
		...
		Point actualPosition = prevPosition + walkDir*forward + Point(0, 1, 0)*up;
		glTranslatef(actualPosition.x, actualPosition.y, actualPosition.z);
